  ...
  logos.map(item => ({
    ...
    node: (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
        <span style={{ fontSize: '42px', color: '#fff', opacity: 0.8, display: 'flex' }}>
          {item.node}
        </span>
        <span style={{
          fontFamily: "'Space Mono', monospace", fontSize: '10px',
          color: 'rgba(255,255,255,0.3)', letterSpacing: '1.5px', whiteSpace: 'nowrap',
        }}>
          {item.title}
        </span>
      </div>
...
const Skills = () => (
  <section id="skills" style={{
    ...
    {/* Row 2 — Backend & Security (opposite direction) */}
    <div style={{ width: '100%' }}>
      <LogoLoop
        logos={row2}
        speed={80}
